...
void tunnel_the_request(void *arg)
{
	...
	ret = tcp_transparent_connect_to_server(g_dest_ip, g_dest_port, &proxy_info->os_socket, inet_ntoa(proxy_info->client_address.sin_addr),
												proxy_info->client_address.sin_port);
	if (ret != 0) {
		log_msg(LOG_ERR, "Error in tcp_transparent_connect_to_server !!!");
		goto label1 ;
	}
