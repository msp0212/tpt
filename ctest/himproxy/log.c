/*
 * This file contains proxy logging part
 */
/*System Headers*/
#include<syslog.h>
#include<stdarg.h>
...
int write_log(pthread_t thread_id, char *file_name,
		int line_number, const char *function_name,
			int log_priority, char *log_format, ...)
{
	va_list log_ap;
	char format[2048] = {0,};
	
	va_start(log_ap, log_format);
	snprintf(format, sizeof(format), "%lu %s:%d %s > %s", thread_id, file_name,
				line_number, function_name, log_format);
